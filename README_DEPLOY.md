...
Добавлены страницы `community.html` и `submit.html`. Форма умеет делать превью YouTube и TikTok по ссылке. Без внешнего бэкенда заявки сохраняются только в браузере пользователя через localStorage. Для настоящей общей публикации подключите Supabase/Firebase и модерацию. Одобренные работы можно вручную добавлять в `edits.js`.
