(function(){
  ...
  function injectUiFixes(){
    if(document.getElementById('uiFixes')) return;
    ...
    style.textContent = `
      ...
      .version-selector{margin:22px 0 4px;padding:18px;border:1px solid var(--border);border-radius:20px;background:var(--surface);box-shadow:var(--shadow-soft);max-width:520px}
      .version-selector-title{display:flex;align-items:center;gap:9px;margin-bottom:12px;font-weight:900;color:var(--text)}
      .version-selector-list{display:flex;flex-wrap:wrap;gap:10px}.version-option{border:1px solid var(--border);background:var(--surface-2);color:var(--text);border-radius:14px;padding:11px 14px;font-weight:900;display:inline-flex;align-items:center;gap:8px;transition:var(--transition)}
      .version-option.active{background:linear-gradient(135deg,var(--primary),#8b5cf6);border-color:transparent;color:#fff}.version-option.disabled{opacity:.58;cursor:not-allowed}.version-note{margin-top:10px;color:var(--muted);font-size:.9rem;font-weight:700}
  ...
  function syncFavoriteUI(){
    ...
    $$('.favorite-btn[data-id]').forEach(btn => {
      ...
      btn.innerHTML = active ? `<i class="fas fa-heart"></i>${detailButton ? ' В избранном' : ''}` : `<i class="far fa-heart"></i>${detailButton ? ' В избранное' : ''}`;
  ...
  function toggleTheme(){ const dark = !document.documentElement.classList.contains('dark-mode'); applyTheme(dark); localStorage.setItem(THEME_KEY, dark ? 'dark' : 'light'); }
  ...
  function filterCatalog(){
    ...
    let visible = 0;
    cards.forEach(card => {
      const show = (active === 'all' || card.dataset.category === active) && (!search || cardText(card).includes(search)) && (!favoritesOnly || favorites.includes(card.dataset.id));
      card.hidden = !show;
      if(show) visible += 1;
    });
    ...
    if(label){ label.textContent = ($('#categoryFilters .filter-btn.active')?.textContent || 'Все категории') + (favoritesOnly ? ' · избранное' : ''); }
  ...
  function initCatalog(){
    ...
    $('#categoryFilters')?.addEventListener('click', event => {
      ...
      button.classList.add('active'); filterCatalog();
    });
    filterCatalog();
  }
  ...
  function initCommunity(){
    ...
    function apply(){
      ...
      const count = $('#editCount'); if(count) count.textContent = `Работ: ${visible}`;
      const empty = $('#communityEmpty'); if(empty) empty.hidden = visible !== 0;
    }
    ...
    $('#editFilters')?.addEventListener('click', event => {
      ...
      button.classList.add('active'); apply();
    });
    apply();
  }
  ...
  function initSubmit(){
    ...
    $('#editSubmitForm')?.addEventListener('submit', event => {
      ...
      const item = { id:'local-' + Date.now(), url:$('#editUrl')?.value || '', title:$('#editTitle')?.value || 'Без названия', author:$('#editAuthor')?.value || 'Автор', plugins:$('#editPlugins')?.value || '', description:$('#editDescription')?.value || '', created:new Date().toISOString() };
      const list = readSubmissions(); list.unshift(item); writeJson(EDIT_SUBMISSIONS_KEY, list); event.target.reset(); renderSubmissions();
    });
  }
  ...
  function initFlowVersionSelector(){
    ...
    if(!content || !actions || !downloadBtn || document.getElementById('flowVersionSelector')) return;
    const box = document.createElement('div');
    box.id = 'flowVersionSelector';
    box.className = 'version-selector reveal revealed';
    box.innerHTML = `
      <div class="version-selector-title"><i class="fas fa-code-branch"></i> Выбор версии Flow</div>
      <div class="version-selector-list">
        <button class="version-option disabled" type="button" disabled><i class="fas fa-clock"></i> v1.4.1 обновляется</button>
        <a class="version-option active" href="/download/flow" target="_blank" rel="noopener"><i class="fas fa-check-circle"></i> Скачать v1.5.2</a>
      </div>
      <div class="version-note">Сейчас доступна версия v1.5.2. Когда будет ссылка на v1.4.1, добавлю её сюда отдельной кнопкой.</div>
    `;
  ...
  function initEvents(){
    ...
    document.addEventListener('click', event => {
      const button = event.target.closest('.favorite-btn[data-id]'); if(!button) return;
      event.preventDefault(); toggleFavorite(button.dataset.id);
    });
    const top = $('#backToTop');
