...
function pluginCard(plugin) {
  ...
  return `<article class="plugin-card reveal" data-plugin-card data-id="${esc(plugin.id)}" data-category="${esc(plugin.category)}" data-status="${esc(plugin.status)}" data-name="${esc(plugin.name)}" data-size="${pluginSizeMB(plugin)}"><div class="plugin-card-header"><div class="badge-stack"><span class="plugin-badge">${esc(plugin.badge || plugin.categoryLabel)}</span>${statusBadge(plugin)}</div><div class="card-actions"><button class="favorite-btn" data-id="${esc(plugin.id)}" type="button" aria-label="Избранное"><i class="far fa-heart"></i></button><div class="plugin-icon"><i class="${esc(plugin.icon || 'fas fa-plug')}"></i></div></div></div><div class="plugin-card-content"><h3>${esc(plugin.name)}</h3><p>${esc(plugin.description)}</p><div class="plugin-tags">${tags}</div><div class="plugin-meta"><span><i class="fas fa-hdd"></i> ${esc(plugin.size)}</span><span><i class="fas fa-calendar"></i> ${esc(plugin.version)}</span><span><i class="fas fa-check-circle"></i> AE ${esc(plugin.minAe)}+</span></div></div><div class="plugin-card-footer">${download}<a class="details-btn" href="/plugin/${url(plugin.id)}"><i class="fas fa-info-circle"></i> Подробнее</a></div></article>`;
}
